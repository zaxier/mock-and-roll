...
For single-dataset requests, run:
```bash
uv --project "$(realpath "${CLAUDE_SKILL_DIR}/../..")" run mock-and-roll suggest \
...
For multi-entity and join-heavy requests, default to authoring `/tmp/<domain>_model.yml` directly (using the model example in [references/spec-guide.md](references/spec-guide.md) as a starting structure), then tailor datasets, columns, row counts, and relationships to the user domain.

Only use `suggest-model` when the user explicitly asks for a quick scaffold:
```bash
uv --project "$(realpath "${CLAUDE_SKILL_DIR}/../..")" run mock-and-roll suggest-model \
  --description "<user description>" \
  --catalog <catalog> \
  --schema <schema> \
  --rows <rows> \
...
```
Treat `suggest-model` output as a rough template and rewrite it to match the requested domain before preview/create.
...
Run:
```bash
uv --project "$(realpath "${CLAUDE_SKILL_DIR}/../..")" run mock-and-roll preview --spec /tmp/<table>_spec.yml --limit 10
```
For model specs:
```bash
uv --project "$(realpath "${CLAUDE_SKILL_DIR}/../..")" run mock-and-roll preview-model --spec /tmp/<domain>_model.yml --limit 5
```

5. Create the Delta table.
Run:
```bash
uv --project "$(realpath "${CLAUDE_SKILL_DIR}/../..")" run mock-and-roll create --spec /tmp/<table>_spec.yml --profile <profile_name>
```
For model specs:
```bash
uv --project "$(realpath "${CLAUDE_SKILL_DIR}/../..")" run mock-and-roll create-model --spec /tmp/<domain>_model.yml --profile <profile_name>
```
